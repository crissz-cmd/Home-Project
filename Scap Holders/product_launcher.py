...
def trade_log_path(): return data_dir()/"trade_log.csv"
def verify_license():
...
class App(tk.Tk):
    ...
    def start(self):
        ...
        os.environ.update({"MT5_TERMINAL_PATH":s["mt5_terminal_path"],"SCAP_SYMBOLS":s["symbol"],"SCAP_RISK_PERCENT":str(s["risk_percent"]),"SCAP_TARGET_PROFIT_USD":str(s["target_profit_usd"]),"SCAP_MAX_OPEN_POSITIONS":str(s["max_open_positions"]),"SCAP_MAX_DAILY_LOSS_PERCENT":str(s["max_daily_loss_percent"]),"SCAP_MAX_SPREAD_POINTS":str(s["max_spread_points"]),"SCAP_DRY_RUN":"1" if s["dry_run"] else "0","SCAP_LOG_FILE":str(trade_log_path()),"SCAP_REQUIRE_LIVE_CONFIRMATION":"0","SCAP_PRODUCT_MODE":"1"})
